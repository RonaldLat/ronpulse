import { o as me, t as we } from '../chunks/scheduler.bdb66109.js';
import {
	S as Ge,
	a as Je,
	I as M,
	...
	P as Me,
	e as Ze
} from '../chunks/singletons.d7c88a27.js';
import { b as q } from '../chunks/paths.01a3d415.js';
function Qe(t, r) {
	return t === '/' || r === 'ignore'
		? t
		: r === 'never'
...
function lt(t, r) {
	...
	if (i != null && i.textContent) {
		...
		return (
			E && ae.set(f, { body: p, init: u, ttl: 1e3 * Number(E) }),
				...
				: new RegExp(
						`^${ht(t)
							.map((i) => {
								...
								if (p)
									return (
										r.push({ name: p[1], matcher: p[2], optional: !1, rest: !0, chained: !0 }),
										'(?:/(.*))?'
									);
								const u = /^\[\[(\w+)(?:=(\w+))?\]\]$/.exec(i);
								if (u)
									return (
										r.push({ name: u[1], matcher: u[2], optional: !0, rest: !1, chained: !0 }),
										'(?:/([^/]+))?'
									);
								if (!i) return;
								const E = i.split(/\[(.+?)\](?!\])/);
			...
			if (typeof l[0] == 'string') {
				...
				switch (g) {
					...
					case 'null':
						...
						i[u] = T;
						for (let R = 1; R < l.length; R += 2) T[l[R]] = p(l[R + 1]);
						break;
					default:
	...
	for (const i in t)
		if (typeof ((f = t[i]) == null ? void 0 : f.then) == 'function')
			return Object.fromEntries(
				await Promise.all(Object.entries(t).map(async ([p, u]) => [p, await u]))
			);
	return t;
}
class ne {
...
function Ot(t, r) {
	...
	x || ((x = Date.now()), history.replaceState({ ...history.state, [M]: x }, '', location.href));
	const fe = J[x];
	...
	function Pe(e) {
		...
		s && s.remove(),
			(H = e.props.page),
			(V = new t.root({ target: r, props: { ...e.props, stores: B, components: l }, hydrate: !0 })),
			Ae(x);
		const n = {
			from: null,
			to: {
	...
	async function X({ url: e, params: s, branch: n, status: o, error: a, route: c, form: h }) {
		...
		return (
			(!d.url || e.href !== d.url.href || d.error !== a || (h !== void 0 && h !== H.form) || L) &&
				(b.props.page = {
					error: a,
					params: s,
	...
	async function ue({ loader: e, parent: s, url: n, params: o, route: a, server_data_node: c }) {
		...
		if ((v = b.universal) != null && v.load) {
			...
			(h = (await b.universal.load.call(null, y)) ?? null), (h = h ? await At(h, a.id) : null);
		}
		return {
			node: b,
			loader: e,
			server: c,
			universal: (L = b.universal) != null && L.load ? { type: 'data', data: h, uses: _ } : null,
	...
	async function pe({ id: e, invalidating: s, url: n, params: o, route: a }) {
		...
		const j = b.map(async (S, I) => {
			...
			if (
				...
				!Oe(O, A, L, (he = k.universal) == null ? void 0 : he.uses, o)
			)
				return k;
	...
	async function ie({ status: e, error: s, url: n, route: o }) {
		...
		return await X({ url: n, params: a, branch: [_, b], status: e, error: s, route: null });
	}
	function Z(e, s) {
		if (_e(e, q)) return;
		const n = se(e);
		for (const o of f) {
			const a = o.exec(n);
			if (a) return { id: e.pathname + e.search, invalidating: s, route: o, params: tt(a), url: e };
		}
	}
	function se(e) {
	...
	function Ue({ url: e, type: s, intent: n, delta: o }) {
		...
		return U || m.before_navigate.forEach((_) => _(h)), a ? null : c;
	}
	async function ce({
		url: e,
		scroll: s,
		...
		var j, F, S;
		const L = Z(e, !1),
			A = Ue({ url: e, type: c, delta: h, intent: L });
		if (!A) {
			v();
			return;
		}
		...
		if (((g = null), T)) {
			...
			const I = (await Promise.all(m.on_navigate.map((k) => k(A.navigation)))).filter(
				(k) => typeof k == 'function'
			);
			if (I.length > 0) {
				let k = function () {
	...
	function Xe() {
		...
		u.addEventListener('mousedown', s), u.addEventListener('touchstart', s, { passive: !0 });
			...
			(history.scrollRestoration = 'manual'),
				...
				addEventListener('popstate', async (n) => {
					...
					if (((W = {}), (o = n.state) != null && o[M])) {
						...
						if (
							((a = d.url) == null ? void 0 : a.href.split('#')[0]) === location.href.split('#')[0]
						) {
							e(h), (J[x] = te()), (x = n.state[M]), scrollTo(c.x, c.y);
							return;
						}
						const _ = n.state[M] - x;
						await ce({
				...
				addEventListener('hashchange', () => {
					C && ((C = !1), history.replaceState({ ...history.state, [M]: ++x }, '', location.href));
				});
...
async function He(t, r) {
	...
	if (
		((p = i.headers.get('content-type')) != null && p.includes('text/html') && (await K(t)), !i.ok)
	)
...
function Ee() {
	...
	if (t) t.focus();
		...
		if (i && i.type !== 'None') {
			const p = [];
			for (let u = 0; u < i.rangeCount; u += 1) p.push(i.getRangeAt(u));
			setTimeout(() => {
